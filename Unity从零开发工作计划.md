...
| M4 | Asset/Definition/Instance 纯数据模型、内容寻址去重、四种位置、堆叠/收纳/附着/解除、旋转/翻面/显隐、关系循环校验、Command/Journal 接入、Workbench 棋子库搜索/实例列表/基础操作 UI、占位投影；EditMode 65/65、PlayMode 2/2 | 图片导入/缩略图、分类标签编辑、堆叠/收纳/附着完整操作 UI、纹理性能基准 | Windows/跨平台矩阵与后续发布门槛 |
...
当前进度：M4 第一批已实现：Asset/Definition/Instance 纯数据模型、内容寻址资产去重、四种互斥位置、堆叠/收纳/附着/解除、旋转/翻面/显隐、关系循环校验、版本化 Command/Journal 接入，以及 Workbench 的棋子库搜索/实例列表/基础操作 UI 和占位棋子投影；EditMode 65/65、PlayMode 2/2。图片导入/缩略图、分类标签编辑、复杂关系操作 UI、纹理性能预算仍在进行，详见 [M4 结果报告](./SundollWorld/Docs/M4-结果报告.md)。
