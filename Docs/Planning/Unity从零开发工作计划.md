...
1. **项目中心**：项目生命周期、最近项目、恢复与便携包；作为弹窗入口，不长期占据主工作台。
2. **地图制作器**：编辑 `MapDocument`，显式发布 `MapContentVersion`。
3. **棋子库**：维护当前项目的定义与视觉资源。
4. **主控台**：创建 `ScenarioDocument`，实例化地图并主持互动。

三个主工作区通过左上角紧凑按钮组切换整个页面，不得只切换左侧窄栏。它们共享同一个 Application 层，但不共享混乱的全局 UI 状态。每个文档拥有自己的 Undo Scope；切到另一张地图后，撤销默认不会修改前一张地图。

### 3.2 Workbench 布局

首版采用稳定、可调宽度的工作台布局：

- 顶部：项目、保存状态、三个主工作区切换与当前情境工具。
- 左侧：当前工作区的地图/图层/素材、棋子筛选或主持地图列表。
- 中央：2D 地图视口。
- 右侧：随选择变化的棋子、地图对象、格子选区或主持状态 Inspector。
- 底部：紧凑状态栏；操作历史、错误、验证和恢复信息使用可展开抽屉。

完整 IDE 式自由 Docking 延期。UI Toolkit 负责面板和表单，地图与棋子由 Unity 2D 场景渲染；不把整张地图做成成千上万个 `VisualElement`。

地图有效区域必须拥有持续可见的外框；地图外区域使用更暗、不可编辑的视觉，地图内外一眼可分。笔刷和粘贴预览到达边界时必须显示越界反馈，任何越界批量操作整体拒绝。正式视觉采用深色专业外壳与暖色地图画布，完整要求见 [Workbench UX 重构计划](../../SundollWorld/Docs/Planning/Workbench-UX重构计划.md)。
...
1. 视口、选择、平移、缩放、网格和吸附。
   - 地图有效区域具有清晰、缩放后仍可见的外框；地图外区域不可编辑并与地图内形成明确明暗层次。
   - 越界笔刷/粘贴 Ghost 显示警告色，批量越界继续保持整批拒绝；提供“适应地图”视口动作。
2. 画笔、橡皮、拾取。
3. 直线、矩形、填充。
4. 框选、多选、复制、剪切、粘贴和旋转。
5. 内容图层和本机图层编辑状态。
...
- 完成 Workbench UX 重构：三个主工作区、明确地图边界、地图工具/图层/素材重排、情境 Inspector 与正式视觉系统。
