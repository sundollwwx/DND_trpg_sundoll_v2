...
2026-08-31 使用最新 macOS Universal IL2CPP Player 做了真实窗口检查。当前
测试桌面可用窗口为 `1280×720`（截图文件像素为 `1280×748`，包含 macOS
标题栏），因此本节只作为可用性和布局证据，不替代 `1440×900` 或
`2560×1440` 验收：

- 顶部 `地图制作 / 棋子库 / 主控台` 三个一级工作区可独立切换，切换后内容区域互不混用。
- 地图制作页显示持续可见的暖色有效区外框、地图外暗色区域和“仅可在框内编辑”提示。
- 棋子库页显示搜索/筛选、定义编辑、素材导入、定义列表和独立的“棋子状态”区域；未选中时显示明确空状态。
- 主控台页显示地图管理、主持状态和层级区域，地图编辑与玩家预览提示可读。
- 中文标题与正文在当前窗口内可读，没有观察到明显截断或重叠；更高分辨率、缩放档位和真实键盘手感仍需后续设备复验。

截图证据：

- `M7-workbench-ux7-player-map-20260831.jpg`
- `M7-workbench-ux7-player-pieces-20260831.jpg`
- `M7-workbench-ux7-player-host-20260831.jpg`
...
- UX7 已完成当前 macOS Player 的键盘焦点、中文布局和三工作区视觉截图复验；`1440×900`、`2560×1440` 和更多缩放档位仍未在本设备复验。
