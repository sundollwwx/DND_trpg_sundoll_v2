...
- CI/build validation: no CI configuration detected. M7 macOS universal IL2CPP build result is Success and the M5 projection-inclusive Player Smoke passed. After removing unused Visual Scripting, a clean temporary build reports `errors=0`, `warnings=1`, `TypeDB: Class` count 0 and no `visualscripting` residue; the remaining warning is Unity Cloud native symbols upload token absence, not script compilation. The current local Library/Bee build still emits legacy TypeDB diagnostics, so release evidence continues to prefer clean imports. Details are in `Docs/M7-结果报告.md` and `Docs/Evidence/M7-clean-build-no-visualscripting-20260827.md`. Latest interactive test evidence is `SundollWorld/TestResults_EditMode_20260825_m5_hierarchy_context.xml` (84/84) and `SundollWorld/TestResults_PlayMode_20260825_m5_hierarchy_context.xml` (13/13); macOS baselines cover batch edit, Snapshot, Revision save, 10,000 Journal recovery, 64-piece texture sharing, 1000-piece projection/allocation, piece-thumbnail LRU behavior, and real-window render capacity, while long-run production pacing, Windows build, independent-process forced-exit recovery and cross-platform save exchange remain unverified.
