...
- CI/build validation: no CI configuration detected. M7 macOS universal IL2CPP build and no-graphics player startup Smoke passed for the previous M7 slice; the M5 projection-inclusive rebuild is the next validation action. Details are in `Docs/M7-结果报告.md`. Latest test evidence is `SundollWorld/TestResults_EditMode_20260825_m5m6m7_projection.xml` and `SundollWorld/TestResults_PlayMode_20260825_m5m6m7_projection.xml`; Windows build, independent-process forced-exit recovery, cross-platform save exchange and formal performance checks remain unverified.
