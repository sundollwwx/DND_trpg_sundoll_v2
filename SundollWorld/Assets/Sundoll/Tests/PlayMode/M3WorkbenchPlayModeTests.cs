...
namespace Sundoll.Tests.PlayMode
{
    public sealed class M3WorkbenchPlayModeTests
    {
        ...
        public IEnumerator WorkbenchPrimaryWorkspacesAreMutuallyExclusiveAndMapBoundaryIsExplicit()
        {
            ...
            var mapWorkspaceButton = document.rootVisualElement.Q<Button>("PrimaryWorkspace_map");
            var pieceWorkspaceButton = document.rootVisualElement.Q<Button>("PrimaryWorkspace_pieces");
            var hostWorkspaceButton = document.rootVisualElement.Q<Button>("PrimaryWorkspace_host");
            Assert.That(mapWorkspaceButton.focusable, Is.True);
            Assert.That(pieceWorkspaceButton.focusable, Is.True);
            Assert.That(hostWorkspaceButton.focusable, Is.True);
            Assert.That(mapWorkspaceButton.tabIndex, Is.EqualTo(1));
            Assert.That(pieceWorkspaceButton.tabIndex, Is.EqualTo(2));
            Assert.That(hostWorkspaceButton.tabIndex, Is.EqualTo(3));
            ...
            Assert.That(hostWorkspace.style.display.value, Is.EqualTo(DisplayStyle.None));
            var pieceSearchField = document.rootVisualElement.Q<TextField>("PieceSearch");
            Assert.That(pieceSearchField, Is.Not.Null);
            pieceSearchField.Focus();
            yield return null;
            Assert.That(root.IsTextInputFocused(), Is.True);
            pieceSearchField.Blur();
            yield return null;
            Assert.That(root.IsTextInputFocused(), Is.False);
