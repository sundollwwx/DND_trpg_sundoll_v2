...
> 状态：**M7 棋子库网格、Unity 连接诊断和固定 batchmode 验证入口已加入；跨平台发布门槛仍未关闭**
> 更新日期：2026-08-27
...
M0 已正式接受并保留 Windows/Finder 限制；Unity `6000.3.22f1` 最新 EditMode 为 84/84，PlayMode 为 13/13，macOS universal IL2CPP 最新代码构建结果为 Success 并通过 Player Smoke。未使用的 Visual Scripting 包已移除；干净临时构建中 TypeDB 重复类型诊断为 0，剩余 1 条 warning 是 Unity Cloud 符号上传 token 为空，不是脚本编译错误。最新测试证据见 [EditMode XML](./SundollWorld/TestResults_EditMode_20260827_221144.xml)、[PlayMode XML](./SundollWorld/TestResults_PlayMode_20260827_221144.xml) 和 [M7 结果报告](./SundollWorld/Docs/M7-结果报告.md)。M7 已有 macOS 批量编辑、Snapshot、Revision 保存、10,000 Journal 恢复、64 棋子纹理共享、1000 棋子投影和稳态分配、棋子库缩略图 LRU 基线；2560×1440 真实窗口 60 FPS、长时间运行以及 Windows/跨平台/强退矩阵仍未关闭。M4 使用新的 `SundollWorld_M4` 本地开发存档根目录，旧 M1/M2/M3 样本原地保留。

Unity 连接和许可证链路排查入口见 [M7 Unity 连接诊断](./SundollWorld/Docs/M7-Unity连接诊断.md)。日常验证优先使用 `scripts/unity-doctor.sh` 和 `scripts/unity-run-tests.sh`，固定走 Unity `6000.3.22f1`，避免 Hub/许可证客户端状态漂移影响判断。
